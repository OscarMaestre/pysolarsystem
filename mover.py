...
def actualizar_incrementos_angulo():
    global incr_angulo
    incr_angulo=[]
...
def on_tecla_pulsada(evento):
    global offset_x_sol, offset_y_sol, rect_sol, centro_pantalla, centro_y, centro_x, escala
    global escala_velocidad

    if evento.type == pygame.KEYDOWN:
        if evento.key == pygame.key.key_code("S"):
            rect_sol.centery+=1
            offset_y_sol=offset_y_sol+1
            centro_y=centro_y+1
        ...
        if evento.key == pygame.K_LEFT:
            if escala_velocidad<0.1:
                return 
            escala_velocidad=escala_velocidad-0.05
            actualizar_incrementos_angulo()
            return
